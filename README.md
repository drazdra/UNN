...
> Unless the model has a separate output vocabulary layer, in which case it has to be compatible with that one instead.

In a way, the whole FFN is like a kick for the tokens to get closer to some specific clouds after Attention has mingled in all the previous "relevant" tokens. In my opinion, it's mostly an error-correction mechanism to fix the results of the MHA amalgamation. No jokes, it's a hard task as it needs to translate the chaos of infused traits back into a recognizable and relevant shape where certain figures would get clear priority. And the key thing is that it can actually learn (and should) the selection of the most relevant clouds of traits. But that's about it. Imho, most of the actual information is stored in the token patterns. And in the way MHA mingles them. Here, imho, we just brush it up, despite the huge size of the block.

There are experiments where people edit weights in FFN and make it produce a different desired output, like changing a city name in response to a question and so on. Yep, why not. They merely change the detection and adjustment of some token cloud traits. The very city name comes from the token vocabulary. That token of the city name has a pattern with its traits. MHA produced something vague. FFN refined the traits properly to align it with that specific city. Engineers managed to find how to maim the refining mechanism to shift its adjustment to a different trait cloud, a wrong one. The city has changed. Does the FFN store facts? I don't think so. How could it even do that, considering that it works *per token*, so it doesn't have *all* the context for the right fact.. it has just a mingled pattern limited with the size of a single token.. sigh.

You could say that the token already has all the context mingled in by MHA, so the relevant the context is in each token! But the thing is, it's only partially true. Just consider the length of your context to the size of a token. Even if you believe a token has all relevant information, the mingled-in traits are *not ordered*. It's not a *sequence* of words that makes sense, it's just a cloud of traits where the model needs to filter away the noise and refine the important traits, polishing some traits, dampening others...
...
Getting back to the conceptual level, as you see, we do *not* store the information or *knowledge* anywhere, in the sense of *facts*, symbols, sentences or whatever. It's not a *store*, it's a *function* that knows how to *create* a store. It simply learns the required adjustments to the pattern representing relatedness to clouds of tokens. It did it in Attention, and now it did it in the FFN. 

Is it redundant? Yep, it is. First, Attention adjusted tokens, and now the FFN adjusted the adjusted tokens.. And that's exactly why we can always *distill* a trained model to get a way more compact form that performs nearly as well as the original. 
