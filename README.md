...
In a way, the whole FFN is like a kick for the tokens to get closer to some specific clouds after Attention has mingled in all the previous tokens. In my opinion, it's mostly an error-correction mechanism to fix the results of the MHA amalgamation. No jokes, it's a hard task as it needs to translate the chaos of infused traits back into a recognizable and relevant shape where certain figures would get clear priority. And the key thing is that it can actually learn (and should) the selection of the most relevant cloud of traits. But that's about it. Imho, most of the actual information is stored in the token patterns. And in the way MHA mingles them. Here, imho, we just brush it up, despite the huge size of the block.
