...
You could say that the token already has all the context mingled in by MHA, so most of the context is in a single token! But the thing is, it's only partially true. And besides, the mingled-in traits are not *ordered*. It's not a sequence of words that makes sense, it's just a cloud of traits where the model needs to filter away the noise and refine the important traits, polishing some traits, dampening others.. And it never means a single word, it can be tied to lots of unrelated things conceptually.
